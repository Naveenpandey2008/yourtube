...
const PLANS = [
  { id: 'free', name: 'Free', price: 0, priceDisplay: '₹0', period: 'forever', color: 'text-gray-400', borderColor: 'border-yt-border', bgColor: 'bg-transparent', buttonClass: 'bg-yt-surface hover:bg-yt-border text-yt-text', features: ['1 download per day', 'Standard video quality', 'Basic search', 'Comments & likes', 'Watch history'], notIncluded: ['Ad-free viewing', 'HD downloads', 'Priority support'] },
  { id: 'bronze', name: 'Bronze', price: 99, priceDisplay: '₹99', period: 'per month', color: 'text-orange-400', borderColor: 'border-orange-500', bgColor: 'bg-orange-500/5', buttonClass: 'bg-orange-500 hover:bg-orange-400 text-white', features: ['5 downloads per day', 'HD video quality', 'Advanced search', 'Comments & likes', 'Watch history', 'Ad-free viewing'], notIncluded: ['Priority support', 'Exclusive content'] },
  { id: 'silver', name: 'Silver', price: 199, priceDisplay: '₹199', period: 'per month', color: 'text-gray-300', borderColor: 'border-gray-400', bgColor: 'bg-gray-400/5', buttonClass: 'bg-gray-500 hover:bg-gray-400 text-white', popular: true, features: ['15 downloads per day', 'Full HD 1080p', 'Advanced search', 'Comments & likes', 'Watch history', 'Ad-free viewing', 'Priority support'], notIncluded: ['Exclusive content'] },
  { id: 'gold', name: 'Gold', price: 499, priceDisplay: '₹499', period: 'per month', color: 'text-yellow-400', borderColor: 'border-yellow-500', bgColor: 'bg-yellow-500/5', buttonClass: 'bg-gradient-to-r from-yellow-500 to-orange-500 hover:opacity-90 text-white', features: ['Unlimited downloads', '4K video quality', 'Advanced search', 'Comments & likes', 'Watch history', 'Ad-free viewing', 'Priority support', 'Exclusive content', 'Early access'], notIncluded: [] },
];
...
export default function PlansSection({ variant = 'full', showHeading = true }: PlansSectionProps) {
  ...
  const handleUpgrade = async (planId: string) => {
    ...
    try {
      ...
      if (!orderData.success) { setError(orderData.error || 'Failed to create order'); setLoading(null); return; }
      ...
      {showHeading && (
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-2 mb-3">
            <Crown size={variant === 'compact' ? 24 : 28} className="text-yellow-400" />
            <h2 className={variant === 'compact' ? 'text-2xl font-bold' : 'text-3xl font-bold'}>Upgrade Your Plan</h2>
          </div>
          {variant === 'full' && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {PLANS.map(plan => (
          <div key={plan.id} className={`relative rounded-2xl border-2 p-5 flex flex-col ${plan.borderColor} ${plan.bgColor} ${success === plan.id ? 'ring-2 ring-green-500' : ''}`}>
            {plan.popular && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-yt-red text-white text-xs font-bold rounded-full">Most Popular</div>
            )}
            {currentPlan === plan.id && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-yt-surface border border-yt-border text-yt-muted text-xs font-medium rounded-full">Current Plan</div>
            )}
            <div className="mb-4">
